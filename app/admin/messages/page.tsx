...
const CONVERSATIONS = [
  {
    ...
    unread: true,
    messages: [
      { id: 'm1', sender: 'admin', text: 'Hello, we have uploaded the initial prior art search results to your document center. Let us know if you have any questions.', timestamp: 'Yesterday, 2:00 PM' },
      { id: 'm2', sender: 'client', text: 'Thanks, reviewing them now. Can we schedule a call to discuss the latest drafts?', timestamp: 'Today, 10:30 AM' }
    ]
  },
  {
    id: '2',
    ...
    unread: false,
    messages: [
      { id: 'm1', sender: 'client', text: 'The new CAD files look great. Proceeding with phase 2.', timestamp: 'Yesterday, 4:15 PM' }
    ]
  },
  {
    id: '3',
...
export default function MessagesPage() {
  ...
  const handleSend = () => {
    ...
    setReplyText('');
    // Mock API Call / Notification
    console.log('Sending message to client via API, notifying them via email...');
  };
          ...
          {filteredConversations.map((chat) => (
            <button
              ...
              onClick={() => {
                ...
                // Mark as read mock
                setConversations(conversations.map(c => c.id === chat.id ? { ...c, unread: false } : c));
              }}
              className={cn(
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                <div className="flex justify-between items-center mb-1">
                  <h3 className={cn("text-sm truncate", chat.unread ? "font-bold text-slate-900 dark:text-white" : "font-medium text-slate-700 dark:text-slate-200")}>
                    {chat.clientName}
                  </h3>
                  <span className="text-xs text-slate-500 shrink-0">{chat.time}</span>
                </div>
                <p className={cn("text-xs truncate", chat.unread ? "text-slate-900 dark:text-white font-medium" : "text-slate-500 dark:text-slate-400")}>
                  {chat.lastMessage}
                </p>
              </div>
              {chat.unread && (
                <div className="w-2.5 h-2.5 rounded-full bg-accent shrink-0 mt-1"></div>
              )}
            </button>
          ))}
        </div>
        ...
        <div className="flex-1 flex flex-col bg-white dark:bg-slate-900">
          ...
          <div className="p-4 border-b border-slate-200 dark:border-slate-800 flex justify-between items-center bg-white dark:bg-slate-900">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h2 className="text-base font-bold text-slate-900 dark:text-white">{activeConversation.clientName}</h2>
                <p className="text-xs text-slate-500">Active Project - Priority Support</p>
              </div>
            </div>
